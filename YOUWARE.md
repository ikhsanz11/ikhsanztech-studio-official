...
- **Project Type**: IkhsanzTech Studio Landing Page (2026 Edition)
- **Entry Point**: `src/main.tsx` (React application entry)
- **Build System**: Vite 7.0.0 (Fast development and build)
- **Styling System**: Tailwind CSS 3.4.17 with custom glassmorphism effects

## IkhsanzTech Studio Design System

### Visual Identity
- **Primary Colors**: Dark (#050505), Neon Cyan (#00F2FF), Electric Purple (#7C3AED)
- **Typography**: Sora (Headlines), Inter (Body)
- **Border Radius**: Extra Rounded (24px - 32px)

### Custom Effects
- **Glassmorphism Cards**: `.glass-card` class with backdrop-filter: blur(20px) and animated gradient border
- **Ambient Gradient**: `.ambient-gradient` for floating background effect
- **CTA Glow**: `.btn-glow` for hover glow effect on buttons
- **Neon Text**: `.neon-text` for text glow effect

### Components Structure
- `Header.tsx` - Fixed navigation with glassmorphism background
- `HeroSection.tsx` - Main hero with ambient gradient and floating particles
- `ShowreelSection.tsx` - Video portfolio preview
- `ServicesSection.tsx` - Horizontal carousel with glass cards (8 services)
- `ProductsSection.tsx` - 4-column grid for digital products
- `TestimonialsSection.tsx` - Auto-scrolling marquee testimonials
